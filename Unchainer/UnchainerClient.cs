...
/// <summary>
/// Provides the methods for creating <see cref="ICredentialsRequest">unified Unchainer credential registration request messages</see>
/// and for handling the <see cref="CredentialsResponse">credential registration responses</see> received from the coordinator.
/// </summary>
public class UnchainerClient
{
	public UnchainerClient(
		...
		CancellationToken cancellationToken)
	{
		...
		{
			throw new UnchainerCryptoException(UnchainerCryptoErrorCode.CredentialToPresentDuplicated);
		}
		...
		CredentialsResponseValidation registrationValidationData)
	{
		...
		if (issuedCredentialCount != NumberOfCredentials)
		{
			throw new UnchainerCryptoException(
				UnchainerCryptoErrorCode.IssuedCredentialNumberMismatch,
		...
		{
			throw new UnchainerCryptoException(UnchainerCryptoErrorCode.ClientReceivedInvalidProofs);
		}
