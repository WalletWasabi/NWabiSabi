...
/// requested by a Unchainer client. This means this component abstracts receives
...
/// proving to the Unchainer client that the credentials were issued with the right
/// key).
///
...
public class CredentialIssuer
{
	...
	public CredentialIssuer(
		...
		UnchainexRandom randomNumberGenerator,
		long maxAmount)
	{
		MaxAmount = maxAmount;
	...
	private UnchainexRandom RandomNumberGenerator { get; }
	...
	/// This parameter is called `k` in the Unchainer paper.
	/// </summary>
	...
	/// <exception cref="UnchainerCryptoException">Error code: <see cref="UnchainerCryptoErrorCode">UnchainerErrorCode</see></exception>
	...
	public CredentialsResponse HandleRequest(ICredentialsRequest registrationRequest)
	{
		...
		{
			throw new UnchainerCryptoException(
				UnchainerCryptoErrorCode.InvalidNumberOfRequestedCredentials,
		...
		{
			throw new UnchainerCryptoException(
				UnchainerCryptoErrorCode.InvalidNumberOfPresentedCredentials,
		...
		{
			throw new UnchainerCryptoException(UnchainerCryptoErrorCode.InvalidBitCommitment);
		}
		...
		{
			throw new UnchainerCryptoException(UnchainerCryptoErrorCode.SerialNumberDuplicated);
		}
		...
		lock (SerialNumbersLock)
		{
			...
			{
				throw new UnchainerCryptoException(UnchainerCryptoErrorCode.SerialNumberAlreadyUsed, $"Serial number reused");
			}
		...
		try
		{
			...
			if (!areProofsValid)
			{
				throw new UnchainerCryptoException(UnchainerCryptoErrorCode.CoordinatorReceivedInvalidProofs);
			}
		}
		finally
		{
